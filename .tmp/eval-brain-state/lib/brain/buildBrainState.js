...
function buildTinaRead({ hasFounderSignal, sourcingReadiness, missingSignals, roleThesis, leads, batchQualityScore, confidenceScore }) {
    ...
    if (sourcingReadiness === "calibration_batch")
        return "Enough signal for a first pass; Tina should filter hard and learn from the batch.";
    if (roleThesis)
        return `One blocker remains before sourcing: ${missingSignals[0] || "the role lane"}.`;
    return EMPTY_READ;
}
