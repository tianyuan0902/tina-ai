...
exports.evaluateSourcingReadiness = evaluateSourcingReadiness;
function evaluateSourcingReadiness(messages) {
...
function inferRoleFamily(text) {
    if (/\b(ai|llm|ml|machine learning|model|agent|engineer|developer|backend|frontend|full stack|full-stack|infra|platform|solidity|smart contract|smartcontract|web3|defi|protocol)\b/.test(text))
        return "engineering";
    if (/\b(pm|product manager|head of product|product lead|product operator|product)\b/.test(text))
        return "product";
    if (/\b(operator|ops|operations|chief of staff|founder office|bizops)\b/.test(text))
        return "operator";
    if (/\b(designer|design|ux|ui)\b/.test(text))
        return "design";
    if (/\b(gtm|sales|account executive|growth|revenue|marketing)\b/.test(text))
        return "gtm";
    return "general";
}
function buildSearchThesis(text, roleFamily) {
