...
import {
  buildExampleShapeFeedback,
  buildExampleShapeFeedbackResponse,
  buildExampleShapeResponse,
  buildExampleShapes,
...
export async function POST(request: Request) {
  ...
  const currentRead = shouldUseRequestCurrentRead(latestUserMessage?.content || "", computedCurrentRead, requestCurrentRead)
    ? requestCurrentRead!
    : computedCurrentRead;
  const currentReadText = formatCurrentReadForPrompt(currentRead);
  const signalMap = requestSignalMap && requestSignalMap.derivedFromThesisTitle === (currentRead.likelyArchetype || currentRead.thesisTitle)
    ? requestSignalMap
    : undefined;
  ...
  if (!shouldRunProfileSearch && isExampleShapeFeedback(latestUserMessage.content) && hasPreviousExampleShapes(cleanMessages)) {
    const exampleShapeFeedback = buildExampleShapeFeedback(latestUserMessage.content, currentRead);
    const responseContent = buildExampleShapeFeedbackResponse(exampleShapeFeedback);

    return NextResponse.json({
      message: {
        id: `tina-example-shape-feedback-${Date.now()}`,
        role: "tina",
        content: responseContent,
  ...
  if (!shouldRunProfileSearch && exampleShapeRequest) {
    const exampleShapes = buildExampleShapes(currentRead, canonicalSearchState, signalMap, latestUserMessage.content);
    const responseContent = buildExampleShapeResponse(exampleShapes);

    return NextResponse.json({
      message: {
        id: `tina-example-shapes-${Date.now()}`,
        role: "tina",
        content: responseContent,
        exampleShapes
      },
  ...
  if (!shouldRunProfileSearch && isRecruitingSystemBlankSignal(cleanMessages)) {
    const responseContent = buildRecruitingSystemBlankResponse();

    return NextResponse.json({
      message: {
        id: `tina-recruiting-system-${Date.now()}`,
        role: "tina",
        content: responseContent
      },
      canonicalSearchState,
  ...
  if (!shouldRunProfileSearch && isRecruitingClarityPushback(cleanMessages)) {
    const responseContent = buildRecruitingClarityResponse();

    return NextResponse.json({
      message: {
        id: `tina-recruiting-clarity-${Date.now()}`,
        role: "tina",
        content: responseContent
      },
      canonicalSearchState,
  ...
  if (shouldRunProfileSearch) {
    ...
    const naturalCalibrationProfileRequest = isCalibrationProfileBatchRequest(latestUserMessage.content, cleanMessages, currentRead, signalMap);
    const canRunNaturalCalibrationBatch = naturalCalibrationProfileRequest && (canRunCalibrationBatch(canonicalSearchState) || hasUsableCurrentRead(currentRead));

    if (explicitSourceRequest && !canRunNaturalCalibrationBatch && needsLocationAlignmentBeforeSourcing(canonicalSearchState, latestUserMessage.content)) {
      return NextResponse.json({
        message: {
    ...
    const allowCalibrationBatch = explicitSourceRequest && (canRunCalibrationBatch(canonicalSearchState) || canRunNaturalCalibrationBatch);
    ...
    const hiringContext = [
      ...
      refinementSummary ? `Example-shape feedback summary for sourcing-brief refinement:\n${refinementSummary}` : "",
    ...
    const isCalibrationProfileBatch = naturalCalibrationProfileRequest && !hasExplicitProfileCount(latestUserMessage.content);
...
function isCapitalAllocationQuestion(message: string) {
  const hasCapitalContext = /\b(\$500k|500k|500,000|capital allocation|budget allocation|budget|runway|cash|capital|allocate|spend|investment)\b/i.test(message);
  const asksAllocation = /\b(what should i do next|what should we do next|how should (i|we) allocate|where should (i|we) spend|what should (i|we) spend)\b/i.test(message);
...
function shouldUseRequestCurrentRead(
  latestUserMessage: string,
  computed: CurrentRead,
  provided?: CurrentRead
) {
  if (!provided) return false;
...
function hasUsableCurrentRead(currentRead?: CurrentRead) {
  return Boolean(
    currentRead &&
...
function buildLocationAlignmentResponse(state: CanonicalSearchState) {
  ...
  return [
    `I’d pause one beat before pulling profiles — geography will change this batch more than another sentence of role nuance.`,
    ...
    `Should I keep this mostly Bay Area, or is remote US fine for now? Once that’s aligned, I’ll use it as search criteria before pulling examples.`
  ].join("\n\n");
}
...
function buildProfileSearchResponse(message: string, metadata: SourcingBatchMetadata, isRefinementSearch = false, isCalibrationProfileBatch = false) {
  ...
  if (isRefinementSearch) {
    return [
      validationLead || `Yeah — I ran another experimental public-profile pass and found ${validCount} new ${validCount === 1 ? "unverified lead" : "unverified leads"} worth reviewing.`,
      `I added ${validCount === 1 ? "it" : "them"} as calibration evidence.${sourceNote}${qualityNote}${filteredNote} Tell me what feels closer or off and I’ll adjust the brief from there.`
    ].join(" ");
  }

  if (isCalibrationProfileBatch) {
    return [
      validationLead || `Yeah — I pulled ${validCount} ${validCount === 1 ? "example" : "examples"} to react to, not finalists.`,
      `I added ${validCount === 1 ? "it" : "them"} as calibration evidence.${sourceNote}${qualityNote}${filteredNote} Don’t outreach yet — tell me what feels close or wrong and I’ll adjust.`
    ].join(" ");
  }

  return [
    validationLead || `Yeah — I ran an experimental public-profile pass and found ${validCount} ${validCount === 1 ? "unverified lead" : "unverified leads"} worth reviewing.`,
    `I added ${validCount === 1 ? "it" : "them"} as calibration evidence.${sourceNote}${qualityNote}${filteredNote} Use these cautiously; tell me what feels on or off and I’ll iterate from there.`
  ].join(" ");
}
...
function hasRecruiterContext(messages: TinaMvpMessage[]) {
  return messages.some((message) =>
    /\b(found(?:ing)? recruiter|first recruiter|recruiter|recruiting|head of talent|talent partner|talent acquisition|candidate pipeline|hiring pipeline|hiring process|hiring system)\b/i.test(message.content)
  );
}

function getLatestFounderText(messages: TinaMvpMessage[]) {
  return [...messages].reverse().find((message) => message.role === "founder")?.content || "";
}

function isRecruitingSystemBlankSignal(messages: TinaMvpMessage[]) {
  if (!hasRecruiterContext(messages)) return false;

  const latest = getLatestFounderText(messages).toLowerCase();
  return /\b(i have nothing|we have nothing|have nothing|no system|no process|no pipeline|no hiring process|no recruiting process|from scratch|zero|nothing yet|that'?s why i need (?:a )?recruiter|need (?:a )?recruiter to help)\b/i.test(latest);
}

function isRecruitingClarityPushback(messages: TinaMvpMessage[]) {
  if (!hasRecruiterContext(messages)) return false;

  const latest = getLatestFounderText(messages).toLowerCase();
  return /\b(it'?s very clear|this is clear|it is clear|clear enough|i already told you|that'?s obvious)\b/i.test(latest);
}

function buildRecruitingSystemBlankResponse() {
  return [
    "Yeah, that is the signal. You are not asking for a recruiter to run a clean process; you are asking for someone to build the hiring system from zero.",
    "I would shape this as a founding recruiting operator or fractional recruiting lead first: clarify the first roles, build the pipeline, set the interview loop, and force fast founder feedback. A pure sourcer will just hand you more noise.",
    "The one thing I would pin down now: how many hires are you trying to make in the next 3-6 months?"
  ].join("\n\n");
}

function buildRecruitingClarityResponse() {
  return [
    "Fair. Then I would stop asking the broad question and make the call: this is a recruiting-system build, not a pure sourcing hire.",
    "The first version should own the hiring plan, candidate pipeline, interview loop, and weekly founder feedback cadence. I would avoid a big-company recruiter who needs clean reqs handed to them.",
    "Next move: define the first two roles and the weekly hiring loop before deciding full-time versus fractional."
  ].join("\n\n");
}
...
function identifyAmbiguousFocalPoint(message: string, state: CanonicalSearchState) {
  ...
  if (/\brecruiter|recruiting|head of talent|talent partner|talent acquisition\b/.test(text)) {
    return {
      observation: `A founding recruiter is not one job until the hiring loop exists.`,
      risk: `If there is no process or pipeline, a pure sourcer will just expose the mess faster.`,
      question: `Are you hiring several people from a clear plan, or trying to build the hiring loop from zero?`
    };
  }
...
function isCalibrationProfileBatchRequest(
  ...
  const asksForSmallTasteBatch =
    ...
    /\b(show me|send me|give me|pull|find|show)\b.*\b(people-ish|peopleish|examples?|what good looks like|right shape|kinda like the right shape)\b/i.test(message) ||
    /\b(i need to see it|need to see it|too abstract|just want to react|react to something|not perfect)\b/i.test(message);
...
function isPublicProfileSearchRequest(message: string) {
  ...
  if (isOperatingPullPhrase(message)) return false;
  if (isExampleShapeRequest(message)) return false;
  ...
  const explicitProfileSearch =
    /\b(search real profiles|experimental sourcing|public profiles?|linkedin profiles?|github profiles?)\b/.test(text) ||
    /\b(show|send|pull|find|look for)\b.*\b(public profiles?|linkedin profiles?|github profiles?|public leads?|public candidates?)\b/.test(text) ||
    /\b(source|sourcing)\b.*\b(candidates?|people|profiles?|leads?|prospects?|targets?)\b/.test(text) ||
    /\b(show me candidates|show candidates|give me candidates|get me candidates|who should we reach out to|who should i reach out to|source candidates|source people|find linkedin profiles|show linkedin profiles|public profile leads)\b/.test(text);

  return explicitProfileSearch && !asksForApproach && !asksForSearchHelpOnly;
}

function hasPreviousExampleShapes(messages: TinaMvpMessage[]) {
  return messages.some((message) => Boolean(message.exampleShapes?.shapes?.length));
}

function isOperatingPullPhrase(message: string) {
  return /\bpull(?:ing)?\s+(people|engineers?|team|folks|resources?)\s+off\b/i.test(message) ||
    /\btake\s+(people|engineers?|team|folks|resources?)\s+off\b/i.test(message);
}
